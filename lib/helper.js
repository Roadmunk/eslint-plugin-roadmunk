...
function hasPropsWithValues(node, attributes) {
	...
	return !_.some(Object.keys(attributes), key => _.get(node, key) !== attributes[key]);
}

function isAnIdentifierWithName(node, subNode, name) {
...
function isPropertyAnIdentifierWithName(node, name) {
	return isAnIdentifierWithName(node, 'property', name);
}

function isObjectAnIdentifierWithName(node, name) {
	return isAnIdentifierWithName(node, 'object', name);
}
