...
module.exports = {
	...
	create : function(context) {
		function isIdentifierWithName(node, name) {
			return hasPropsWithValues(node, { 'property.type' : 'Identifier', 'property.name' : name });
		}
		...
		return {
			MemberExpression(node) {
				 ...
				 */
				if (isIdentifierWithName(node, 'equal') && isIdentifierWithName(node.object, 'to') && hasPropsWithValues(node, { 'object.object.type' : 'CallExpression' })) {
					...
					if (expectCallArgs.length > 1 || expectCallArgs[0].type !== 'MemberExpression' || !isIdentifierWithName(expectCallArgs[0], 'length')) {
						return;
					}
